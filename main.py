...
if __name__ == '__main__':
    ...
    training_time=['2018-01-01 01:00:00','2018-06-30 23:00:00']
    ...
    training_data = training_data.loc[training_time[0]:training_time[1]]
    coins_data = training_data[coins]
    training_data.drop(coins,axis=1)
    coins_data.dropna(axis=1)
    training_data=pd.concat([training_data,coins_data],axis=1)
    training_data.dropna()

    features_chart_data = ['o_t', 'open', 'high', 'low', 'close', 'volume']
    chart_data = training_data[features_chart_data]

    training_data.drop(features_chart_data,axis=1)
    ...
    '''
    policy_learner = PolicyLearner(
        stock_code=stock_code, chart_data=chart_data, training_data=training_data, delayed_reward_threshold=0.01, lr=.1)
